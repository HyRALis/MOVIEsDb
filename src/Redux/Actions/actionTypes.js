export const fetchPopuMovies = "FETCH_POPULAR_MOVIES";
export const fetchRatedMovies = "FETCH_RATED_MOVIES";
export const fetchTrendingMovies = "FETCH_TRENDING_MOVIES";

export const fetchPopuSeries = "FETCH_POPULAR_SERIES";
export const fetchRatedSeries = "FETCH_RATED_SERIES";
export const fetchTrendingSeries = "FETCH_TRENDING_SERIES";

export const fetchTrendingAll = "FETCH_TRENDING_ALL";

export const fetchMoviesGenres = "FETCH_MOVIES_GENRES";
export const fetchSeriesGenres = "FETCH_SERIES_GENRES";

export const fetchItemMovies = "FETCH_ITEM_MOVIES";
export const fetchItemSeries = "FETCH_ITEM_SERIES";
export const setUrlParams = "SET_URL_PARAMS";
export const removeUrlParams = "REMOVE_URL_PARMS";

export const landing = "HOME";
export const movies = "MOVIES";
export const series = "SERIES";
export const artists = "ARTISTS";
export const details = "DETAILS";

export const slideLeft = "SLIDE_LEFT";
export const slideRight = "SLIDE_RIGHT";

export const resizeWindow = "RESIZE_WINDOW";
